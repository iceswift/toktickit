...
import { AuthUser, Category, getCategories, getMyTickets, MyTicketsQuery, RequestedPriority, TicketListResult, TicketStatus } from "./api.js";

const statuses: TicketStatus[] = ["NEW", "OPEN", "IN_PROGRESS", "WAITING_FOR_REQUESTER", "RESOLVED", "CLOSED", "REOPENED", "CANCELLED"];
function statusLabel(status: TicketStatus) { return status.toLowerCase().split("_").map((word) => word.charAt(0).toUpperCase() + word.slice(1)).join(" "); }
...
export function MyTickets({ requester, onCreateTicket, onOpenTicket }: { requester: AuthUser; onCreateTicket: () => void; onOpenTicket: (ticketId: number) => void }) {
  ...
  return <section aria-labelledby="my-tickets-heading">
    <div className="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
      <div><h1 className="h3 mb-1" id="my-tickets-heading">My Tickets</h1><p className="text-secondary mb-0">Tickets submitted by {requester.name}.</p></div>
    ...
    <section className="card shadow-sm mb-4" aria-label="Ticket filters"><div className="card-body">
      <div className="row g-3">
        ...
        <div className="col-12 col-sm-6 col-md-3"><label className="form-label" htmlFor="ticket-status">Current Status</label><select id="ticket-status" className="form-select" value={query.currentStatus ?? ""} onChange={(event) => update("currentStatus", event.target.value ? event.target.value as TicketStatus : undefined)}><option value="">All statuses</option>{statuses.map((status) => <option key={status} value={status}>{statusLabel(status)}</option>)}</select></div>
    ...
    {state === "ready" && result && result.items.length > 0 && <>
      <div className="table-responsive d-none d-md-block"><table className="table align-middle"><thead><tr><th>Ticket Number</th><th>Summary</th><th>Category</th><th>Requested Priority</th><th>Current Status</th><th>Last Updated</th><th><span className="visually-hidden">Actions</span></th></tr></thead><tbody>{result.items.map((ticket) => <tr key={ticket.id}><td className="fw-semibold">{ticket.ticketNumber}</td><td>{ticket.summary}</td><td>{ticket.category.name}</td><td><span className="badge text-bg-secondary">{priorityLabel(ticket.requestedPriority)}</span></td><td><span className="badge text-bg-success">{statusLabel(ticket.currentStatus)}</span></td><td>{formatDate(ticket.updatedAt)}</td><td><button className="btn btn-sm btn-outline-success" onClick={() => onOpenTicket(ticket.id)}>View details</button></td></tr>)}</tbody></table></div>
      <div className="d-md-none">{result.items.map((ticket) => <article className="card mb-3" key={ticket.id}><div className="card-body"><div className="fw-semibold">{ticket.ticketNumber}</div><h2 className="h6 mt-2">{ticket.summary}</h2><p className="mb-2 text-secondary">{ticket.category.name} · {priorityLabel(ticket.requestedPriority)} · {statusLabel(ticket.currentStatus)}</p><small>Last updated {formatDate(ticket.updatedAt)}</small><button className="btn btn-outline-success w-100 mt-3" onClick={() => onOpenTicket(ticket.id)}>View details</button></div></article>)}</div>
