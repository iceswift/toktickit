...
describe("Administrator User Management", () => {
  it("lists users with filters and management actions", async () => {
    ...
    expect((await screen.findAllByText("Amina User"))[0]).toBeInTheDocument();
    ...
    expect(within(screen.getByRole("table")).getByRole("button", { name: "Edit" })).toBeInTheDocument(); expect(within(screen.getByRole("table")).getByRole("button", { name: "Reset password" })).toBeInTheDocument();
  });
  ...
  it("edits a user's role and active state", async () => {
    ...
    render(<UserManagement user={administrator} />); await screen.findAllByText("Amina User"); await user.click(within(screen.getByRole("table")).getByRole("button", { name: "Edit" }));
  ...
  it("creates a user and resets an initial password", async () => {
    ...
    render(<UserManagement user={administrator} />); await screen.findAllByText("Amina User");
    ...
    await user.click(within(screen.getByRole("table")).getByRole("button", { name: "Reset password" })); await user.type(screen.getByLabelText("New initial password"), "Replacement!2026"); await user.click(screen.getByRole("button", { name: "Reset initial password" }));
